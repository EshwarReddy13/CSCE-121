...
void processPostsByUser(Network& cppeers) {
  ...
  vector<Post*> postss = cppeers.getPostsByUser(username);
  int size = postss.size();
  for (int i = 0; i < size; i++) { 
    cout << postss[i] -> getPostText() << endl;
  }
}
...
void processPostsWithHashtags(Network& cppeers) {
  ...
  vector<Post*> postss = cppeers.getPostsWithTag(hashTag);
  int size = postss.size();
  for (int i = 0; i < size; i++) {
    cout << postss[i] -> getPostText() << endl;
  }
...
void processMostPopularHashtag(Network& cppeers) {
  // TODO(student): implement
  vector<string> hashTags = cppeers.getMostPopularHashtag();
