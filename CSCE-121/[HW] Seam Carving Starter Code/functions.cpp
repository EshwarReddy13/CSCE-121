...
int energy(const Pixel *const*image, int col, int row, int width, int height)
{
  ...
  int rx = &image[col1][row1].r - &image[col2][row1].r;
  int gx = &image[col1][row1].g - &image[col2][row1].g;
  int bx = &image[col1][row1].b - &image[col2][row1].b;

  int deltaX = (rx*rx)+(gx*gx)+(bx*bx);
  
  int ry = &image[col1][row1].r - &image[col1][row2].r;
  int gy = &image[col1][row1].g - &image[col1][row2].g;
  int by = &image[col1][row1].b - &image[col1][row2].b;

  int deltaY = (ry*ry)+(gy*gy)+(by*by);

  int energyy = deltaX + deltaY;
